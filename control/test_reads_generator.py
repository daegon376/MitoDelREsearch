...
if __name__ == '__main__':
    ...
    # НАСТРОЙКИ
    read_length = int(100)  # длина генерируемых ридов
    del_length = random.randint(1, 16000)  # длина делеции

    total_amount = int(10000)  # сумма всех ридов
    del_reads_percentage = int(2)  # процентная доля ридов с делецией

    refseq = str('')  # загружаем референсную последовательность в

    with open('sequence.fasta', 'r') as f:
        for s in f:
            refseq = s[:-1]

    end_of_seq_index = int(len(refseq) - 1)
    del_start = int(random.randint(0, end_of_seq_index))  #
    del_end = int(del_start + del_length) % len(refseq)  # координаты конца делеции на кольце
    circular_seq = str()
    deletion = str()
    if del_end > del_start:
        circular_seq = refseq[del_end:] + refseq[:del_start]
        deletion = refseq[del_start:del_end]
    elif del_end < del_start:
    ...
    if len(circular_seq) != len(refseq) - del_length and len(deletion) != del_length:
        input('Error: circular molecule length is wrong!')

    with open('deletion_info.txt', 'w') as f:
        f.write('')
    ...
    reads_ready = int()
    del_reads_amount = 0
    while reads_ready < total_amount:
        reads_ready += 1
        with open('test_reads.txt', 'a') as output:
            read = str()
            if random.randint(1, 100) > del_reads_percentage:
